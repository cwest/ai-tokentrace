...
from ai_tokentrace.services import AsyncFirestoreTokenUsageService


async def verify_document(project_id: str, collection_name: str):
...
async def main():
    """
    Generates content and saves token usage to Firestore asynchronously.
    """
    ...
    print("--- Running Async Firestore Service Example ---")

    project_id = os.getenv("GOOGLE_CLOUD_PROJECT", "demo-project")
    collection_name = "example_token_usage_async"

    # Initialize the async service
    service = AsyncFirestoreTokenUsageService(collection_name=collection_name)
    print(f"Token usage will be saved to Firestore collection: {collection_name}")
    ...
    async with TrackedGenaiClient(service=service) as client:
        try:
            response = await client.aio.models.generate_content(
                ...
                contents="Tell me a joke about asynchronous programming.",
            )
    ...
    await verify_document(project_id, collection_name)
